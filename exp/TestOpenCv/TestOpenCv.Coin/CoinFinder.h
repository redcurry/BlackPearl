...
{
public:
	cv::RotatedRect findCoin(const cv::Mat& image) const;

private:

	cv::Mat median(const cv::Mat& image) const;
	cv::Mat adaptiveThreshold(const cv::Mat& image) const;
	std::vector<cv::RotatedRect> findCoinCircles(const cv::Mat& image) const;
